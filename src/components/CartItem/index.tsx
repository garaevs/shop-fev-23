...
import { addCartSlice, deleteCartSlice } from '../../store/items/cartSlice';
...
const CartItem: React.FC<Items> = ({ title, price, imgUrl, id, fav }) => {
	...
	// 	console.log(favItem, 'fav add click');
	// };

	const deleteCartItem = (id: number) => {
		const idItem = cartItems.filter(item => item.id !== id);
		dispatch(deleteCartSlice(idItem));
	};
	return (
		<div className={styles.item}>
			{/* <div
				className={fav ? styles.fav + ' ' + styles.svg_active : styles.fav}
				onClick={() => onAddFav(id)}>
